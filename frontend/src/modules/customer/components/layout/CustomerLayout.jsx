...
const CustomerLayout = ({ children, showHeader: showHeaderProp, fullHeight = false, showCart: showCartProp, showBottomNav: showBottomNavProp }) => {
    ...
    const hideHeaderRoutes = ['/', '/categories', '/orders', '/transactions', '/profile', '/profile/edit', '/wishlist', '/addresses', '/wallet', '/rewards', '/support', '/privacy', '/about', '/terms', '/refund-policy', '/return-policy', '/shipping-policy', '/checkout', '/search', '/chat', '/stores'];
    ...
    const hideFooterMessageRoutes = ['/profile', '/profile/edit', '/stores'];
    const showFooterMessage = showBottomNav && !hideFooterMessageRoutes.includes(path) && !path.startsWith('/category') && !path.startsWith('/stores');
