...
/**
 * Creates a set of standard CRUD handlers for a Seller model.
 * Automatically scopes all queries to the logged-in seller.
 */
export const createSellerController = (Model, options = {}) => {
    const { 
        populateFields = [], 
        searchFields = ['name'],
        defaultSort = { createdAt: -1 },
        sellerField = 'sellerId', // Field to scope by
        beforeCreate,
        afterCreate,
        beforeUpdate,
        afterUpdate,
    ...
    return {
        ...
        getById: async (req, res) => {
            try {
                ...
                const item = await dbQuery;
                if (!item) return handleResponse(res, 404, `${Model.modelName} not found or access denied`);
